...
void TestRedis() {
   ...
   //redis默认监听端口为6379 可以再配置文件中修改
    ...
    // const char* redis_password = "123456";  // 密码是const char*类型，不然出错
...
// 该测试是封装好的Redis类，单例测试，在单线程是安全的，多线程就危险了(有人find，有人delete) ==》RedisPool
void TestRedisMgr1() {
    //assert(RedisMgr::GetInstance()->Connect("81.68.86.146", 6380));
    assert(RedisMgr::GetInstance()->Auth("123456"));
    assert(RedisMgr::GetInstance()->Set("blogwebsite", "yan.club"));
    std::string value = "";
    assert(RedisMgr::GetInstance()->Get("blogwebsite", value));
    assert(RedisMgr::GetInstance()->Get("nonekey", value) == false);
    assert(RedisMgr::GetInstance()->HSet("bloginfo", "blogwebsite", "yan.club"));
